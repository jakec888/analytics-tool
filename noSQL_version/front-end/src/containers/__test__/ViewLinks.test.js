...
describe("<ViewLinks /> Test", () => {
  ...
  it("should shows seven links", async () => {
    ...
    // const { getByTestId } = render(<ViewLinks />);

    // expect(getByTestId("link-loading")).toHaveTextContent("Loading...");

    const renderedData = await waitForElement(() => getByTestId("link-data"));

    expect(axiosMock.get).toHaveBeenCalledTimes(1);
    expect(renderedData).toHaveLength(7);
  });
});
...
//       expect(wrapper.find("div.list-group-item").length).toEqual(7);
